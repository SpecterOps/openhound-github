...
class EnterpriseTeamMember(BaseAsset):
    ...
    def role_node_id(self) -> str:
        return f"{enterprise_team_node_id(self.enterprise_node_id, self.team_id)}_members"
...
def flatten_enterprise_member(
    member: dict[str, Any], enterprise_node_id: str, enterprise_slug: str
) -> tuple[dict[str, Any] | None, dict[str, Any] | None]:
    typename = member.get("__typename")
    if typename == "EnterpriseUserAccount":
        managed = {
            **member,
            "enterprise_node_id": enterprise_node_id,
            "enterprise_slug": enterprise_slug,
        }
        backing_user = member.get("user")
        user = None
        if backing_user and backing_user.get("id"):
            user = {
                **backing_user,
                "enterprise_node_id": enterprise_node_id,
                "enterprise_slug": enterprise_slug,
