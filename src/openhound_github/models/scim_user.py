...
The SCIM nodes and target-system provisioning edges follow the public
SpecterOps SCIM extension contract. The IdP-to-SCIM edges are intentionally
isolated behind ``emit_legacy_correlation``. They reproduce GitHound's current
Okta correlation behavior and can be removed when a dedicated hybrid
``openhound-scim`` correlator owns that relationship.
"""
...
def scim_organization_id(scope_node_id: str) -> str:
    """Return the stable SCIM container ID for one GitHub collection scope."""
...
class ScimScopeAsset(BaseAsset):
    """Shared scope resolution for enterprise and organization SCIM assets."""
