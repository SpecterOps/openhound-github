...
def organizations(ctx: SourceContext):
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        org_data = org_ctx.client.get(f"/orgs/{org_ctx.org_name}").json()
        org_ctx.org_node_id = org_data.get("node_id")

        actions = org_ctx.client.get(
            f"/orgs/{org_ctx.org_name}/actions/permissions"
        ).json()
        self_hosted_runners = org_ctx.client.get(
            f"/orgs/{org_ctx.org_name}/actions/permissions/self-hosted-runners"
        ).json()
        workflow_perms = org_ctx.client.get(
            f"/orgs/{org_ctx.org_name}/actions/permissions/workflow"
        ).json()
...
def org_roles(ctx: SourceContext, orgs: list[dict] | None = None):
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        ...
        for page in org_ctx.client.paginate(
            f"/orgs/{org_ctx.org_name}/organization-roles", params={"per_page": 100}
        ):
            for role in page:
                yield {
                    **role,
...
def app_installations(ctx: SourceContext):
    ...
    """
    for org_ctx in _iter_collection_contexts(ctx):
        for page in org_ctx.client.paginate(f"/orgs/{org_ctx.org_name}/installations"):
            for item in page:
                yield {
                    **item,
                    "org_node_id": org_ctx.org_node_id,
...
def users(ctx: SourceContext) -> Iterator[dict[str, Any]]:
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        ...
        data = {
            ...
            "variables": {"login": org_ctx.org_name, "count": 100, "after": None},
        }

        for page_data in org_ctx.client.paginate(
            "/graphql",
            method="POST",
            json=data,
            paginator=paginator,
...
def repositories(ctx: SourceContext):
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        ...
        runner_settings = org_ctx.client.get(
            f"/orgs/{org_ctx.org_name}/actions/permissions/self-hosted-runners"
        ).json()
        ...
        if actions.get("enabled_repositories") == "selected":
            ...
            for page in org_ctx.client.paginate(
                f"/orgs/{org_ctx.org_name}/actions/permissions/repositories",
                params={"per_page": 100},
                data_selector="repositories",
            ):
        ...
        if runner_settings.get("enabled_repositories") == "selected":
            ...
            for page in org_ctx.client.paginate(
                f"/orgs/{org_ctx.org_name}/actions/permissions/self-hosted-runners/repositories",
                params={"per_page": 100},
                data_selector="repositories",
            ):
        ...
        for page in org_ctx.client.paginate(
            f"/orgs/{org_ctx.org_name}/repos", params={"per_page": 100}
        ):
            for repo in page:
...
def repository_roles_base(ctx: SourceContext):
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        for page in org_ctx.client.paginate(
            f"/orgs/{org_ctx.org_name}/custom-repository-roles",
            params={"per_page": 100},
        ):
            for role in page:
                yield {
...
def org_runners(ctx: SourceContext):
    for org_ctx in _iter_collection_contexts(ctx):
        for page in org_ctx.client.paginate(
            f"/orgs/{org_ctx.org_name}/actions/runners",
            params={"per_page": 100},
        ...
        for group_page in org_ctx.client.paginate(
            ...
            for group in group_page:
                accessible_repo_node_ids = _runner_group_repo_node_ids(
                    group, org_ctx, org_repos
                )
                try:
                    for runner_page in org_ctx.client.paginate(
                        f"/orgs/{org_ctx.org_name}/actions/runner-groups/{group['id']}/runners",
                        params={"per_page": 100},
                        data_selector="runners",
                    ):
...
def organization_secrets(ctx: SourceContext):
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        for page in org_ctx.client.paginate(
            f"/orgs/{org_ctx.org_name}/actions/secrets", params={"per_page": 100}
        ):
            for item in page:
                yield {
                    **item,
...
def organization_variables(ctx: SourceContext):
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        for page in org_ctx.client.paginate(
            f"/orgs/{org_ctx.org_name}/actions/variables", params={"per_page": 100}
        ):
            for item in page:
                yield {
                    **item,
...
def secret_scanning_alerts(ctx: SourceContext):
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        for page in org_ctx.client.paginate(
            f"/orgs/{org_ctx.org_name}/secret-scanning/alerts",
            params={"per_page": 100},
        ):
            for alert in page:
...
def personal_access_tokens(ctx: SourceContext):
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        for page in org_ctx.client.paginate(
            f"/orgs/{org_ctx.org_name}/personal-access-tokens",
            params={"per_page": 100},
        ):
            for pat in page:
                yield {
...
def personal_access_token_requests(ctx: SourceContext):
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        for page in org_ctx.client.paginate(
            f"/orgs/{org_ctx.org_name}/personal-access-token-requests",
            params={"per_page": 100},
        ):
            for request in page:
                yield {
...
def saml_provider(ctx: SourceContext):
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        ...
        response = org_ctx.client.post("/graphql", json=data).json()
        ...
        yield {
            **idp,
            "org_node_id": org.get("id") or org_ctx.org_node_id,
            "org_name": org.get("name") or org_ctx.org_name,
            "org_login": org_ctx.org_name,
        }
...
def scim_users(ctx: SourceContext):
    ...
    for org_ctx in _iter_collection_contexts(ctx):
        ...
        try:
            for page in org_ctx.client.paginate(
                f"/scim/v2/organizations/{org_ctx.org_name}/Users",
