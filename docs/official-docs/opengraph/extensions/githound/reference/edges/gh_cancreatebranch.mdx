...
| Start | Kind | End |
|-------|-----------|-------|
| [GH_RepoRole](/opengraph/extensions/githound/reference/nodes/gh_reporole) | GH_CanCreateBranch | [GH_Repository](/opengraph/extensions/githound/reference/nodes/gh_repository) |
...
A wildcard BPR blocks creations. The [GH_PushProtectedBranch](/opengraph/extensions/githound/reference/edges/gh_pushprotectedbranch) permission bypasses the push gate regardless of `enforce_admins`.
...
User or Team listed in the wildcard BPR's `pushAllowances` can create branches. This is a per-actor delta edge — only emitted when the actor's role doesn't already grant [GH_CanCreateBranch](/opengraph/extensions/githound/reference/edges/gh_cancreatebranch).
