...
This means one `${{ vars.MY_VAR }}` expression can produce up to two [GH_UsesVariable](/opengraph/extensions/githound/reference/edges/gh_usesvariable) edges — one to the repo-level variable and one to the org-level variable.
